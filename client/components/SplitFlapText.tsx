...
export interface SplitFlapTextProps extends React.HTMLAttributes<HTMLSpanElement> {
  words?: string[];
  text?: string;
  flipDuration?: number;
  stagger?: number;
...
const SplitFlapText: React.FC<SplitFlapTextProps> = ({
  ...
  useEffect(() => {
    ...
    const animateTo = (targetPhrase: string) => {
      ...
      const plans: TilePlan[] = targetChars
        ...
        .filter((plan): plan is TilePlan => plan !== null);
