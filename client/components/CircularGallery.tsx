...
function autoBind(instance: object) {
  ...
  Object.getOwnPropertyNames(proto).forEach(key => {
    ...
    if (key !== 'constructor' && typeof prop === 'function') {
      (instance as Record<string, unknown>)[key] = prop.bind(instance);
    }
  });
}
...
function createPlaceholderImage(text: string): string {
  ...
  if (ctx) {
    ...
    ctx.fillText(text || 'Theme Track', 400, 300);
  }
...
function createTextTexture(gl: any, text: string, font = 'bold 30px monospace', color = 'black') {
  ...
  if (!context) {
    ...
    return { texture, width: 100, height: 30 };
  }
  context.font = font;
...
class Title {
  ...
  createMesh() {
    ...
    const textHeight = Math.min(this.plane.scale.y * 0.13, 1.3);
    const textWidth = textHeight * aspect;
    this.mesh.scale.set(textWidth, textHeight, 1);
    this.mesh.position.y = -this.plane.scale.y * 0.5 - textHeight * 0.5 - 0.14;
...
class Media {
  ...
  createShader() {
    ...
    const fallbackSrc = createPlaceholderImage(this.text || 'Theme');
    const img = new Image();
    ...
    img.src = this.image || fallbackSrc;
  }
  ...
  update(scroll: any, direction: string) {
    ...
    // Gradually decrease opacity towards left and right ends, increasing to 1 in center:
    const normX = Math.min(Math.abs(x) / Math.max(H, 1), 1.25);
    const opacity = Math.max(0.12, Math.min(1.0, 1.0 - Math.pow(normX, 1.35) * 0.88));
    if (this.program?.uniforms?.uOpacity) {
      this.program.uniforms.uOpacity.value = opacity;
    }
    if (this.title?.mesh?.program?.uniforms?.uOpacity) {
      this.title.mesh.program.uniforms.uOpacity.value = opacity;
    }
  ...
  onResize({ screen, viewport }: any = {}) {
    ...
    // Number of cards visible across screen:
    // Mobile: ~1.25 cards (swipe one-by-one)
    // Tablet: ~2.3 cards
    // Desktop: ~4.0 cards (around 4 to 5 cards visible on screen)
    const cardsOnScreen = screenWidth < 640
      ? 1.25
      : screenWidth < 1024
        ? 2.3
        : 4.0;

    // Small, tight gap between cards (~6% of slot width)
    const gapRatio = 0.06;
    const slotWidth = this.viewport.width / cardsOnScreen;
    const cardWidth = slotWidth * (1 - gapRatio);
    const gap = slotWidth * gapRatio;

    // Substantially increased height (up to 72% of viewport height)
    const cardHeight = Math.min(cardWidth * 0.72, this.viewport.height * 0.72);
    ...
    if (this.title && this.title.mesh) {
      const aspect = this.title.width / (this.title.height || 1);
      const textHeight = Math.min(this.plane.scale.y * 0.12, 1.4);
      const textWidth = textHeight * aspect;
      this.title.mesh.scale.set(textWidth, textHeight, 1);
      this.title.mesh.position.y = -this.plane.scale.y * 0.5 - textHeight * 0.5 - 0.15;
    }
  }
}
...
class App {
  ...
  createMedias(items: any[], bend = 1, textColor: string, borderRadius: number, font: string) {
    const defaultItems = [
      { image: `https://picsum.photos/seed/1/800/600?grayscale`, text: 'Robotics' },
      { image: `https://picsum.photos/seed/2/800/600?grayscale`, text: 'HealthcareTech' },
      { image: `https://picsum.photos/seed/3/800/600?grayscale`, text: 'Mobility' },
      { image: `https://picsum.photos/seed/4/800/600?grayscale`, text: 'Green Tech' }
    ];
